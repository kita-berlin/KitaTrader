...
class MacdHistogram(IIndicator):
    """
    Moving Average Convergence Divergence (MACD) Histogram
    
    The MACD is a trend-following momentum indicator. The Histogram represents 
    ...
    def __init__(
        ...
        # Create result DataSeries (Histogram is actually MACD line in cTrader)
        self.histogram = DataSeries(parent_bars, size)
        
        # Create output DataSeries for MACD and Signal
        self.macd = DataSeries(parent_bars, size)
        self.signal = DataSeries(parent_bars, size)
        
        # The classic Histogram bars (MACD - Signal)
        self.difference = DataSeries(parent_bars, size)
    ...
    def calculate(self, index: int) -> None:
        """
        Calculate MACD Histogram for the given index
        ...
        # In cTrader MacdHistogram, the primary output 'Histogram' is actually the MACD line
        self.histogram.data[index] = self.macd.data[index]
        ...
        if signal_val is not None:
            ...
            # The classic "Histogram" bars are the difference
            if not math.isnan(signal_val):
                self.difference.data[index] = self.macd.data[index] - signal_val
